...
{
    [TelegramBotCommand("/searchArr")]
    public class SearchArrCommand : ICommand
    {
        private readonly ILogger<SearchArrCommand> _logger;
        private readonly ITelegramBotClient _bot;
        private readonly BotContext _database;
        private readonly string _apiUrl;

        public SearchArrCommand(IConfiguration configuration, ILogger<SearchArrCommand> logger, ITelegramBotClient bot,
            BotContext database)
        {
            _logger = logger;
            _bot = bot;
        ...
        private async Task<IEnumerable<Showcase>> GetTopFiveVideos(string videoName, string arr, int maxRes)
        {
            var httpClient = new HttpClient();

            var response = await httpClient.GetAsync(_apiUrl + $"/Youtube/SearchByArragment?Search={videoName}&arragment={arr}&MAXRESULT={maxRes}");
        ...
        public async Task ExecuteCommand(Update eventArgs)
        {
            ...
            var data = eventArgs.Message.Text.Replace(Name + " ", "").Split(" ");

            var showCases = await GetTopFiveVideos(data[0], data[1], int.Parse(data[2]));
            ...
            await _bot.SendTextMessageAsync(chatId, $"Choose video to add to the latest playlist {data[0]}\n",
                replyMarkup: markup);
        }
        ...
        public async Task CallBackHandler(Update eventArgs)
        {
            ...
            await _bot.SendTextMessageAsync(eventArgs.CallbackQuery.From.Id,
                $"Video with name - {vidName} was added to library");
